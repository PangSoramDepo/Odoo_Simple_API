...
class API_Route_Controller(http.Controller):
    ...
    @http.route(_AULR+'<string:model>/all',type='json',methods=["GET"], auth='user')
    ...
    @http.route(_AULR+'<string:model>/where',type='json',methods=["GET"], auth='user')
    def model_where(self,model):
        return Utils(model).where('id','=',40).take(limit).get()

    @http.route(_AULR+'<string:model>/add',type='json',methods=["POST"], auth='user')
    ...
    @http.route(_AULR+'<string:model>/update/<int:id>',type='json',methods=["POST"], auth='user')
